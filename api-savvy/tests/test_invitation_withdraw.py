...
def test_user_withdraws_invitation(
    client: TestClient, seed_pending_invitation: Session
):
    headers = {"Authorization": f"JWT {create_access_token('1')}"}

    response = client.delete(
        f"/v1/groups/invitations/4/",
        headers=headers,
    )
...
def test_user_cannot_withdraw_after_invitation_was_rsvp_d(
